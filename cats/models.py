...
def cat_picture_path(instance, filename):
    ...
    return "user/%s/%s" % (userstring, filename)
...
class Profile(models.Model):
    ...
    location = models.CharField(max_length=50, choices=countries, default="CA")
...
class Cat(models.Model):
    ...
    sex = models.CharField(
        max_length=1, choices=(("F", "F"), ("M", "M"), ("X", "X")), default="F"
    )
...
class Match(models.Model):
    ...
    def get24hourtime(self, the_timezone):
        ...
        return (
            str(the_time.hour).zfill(2) + ":" + str(the_time.minute).zfill(2)
        )
...
class Vote(models.Model):
    ...
    value = models.IntegerField(
        blank=False, choices=((1, 1), (-1, -1)), default=1
    )
    voter = models.ForeignKey(
        Cat, on_delete=models.CASCADE, related_name="voter", default=0
    )
    votee = models.ForeignKey(
        Cat, on_delete=models.CASCADE, related_name="votee", default=0
    )
...
def register_match(sender, instance, **kwargs):
    ...
    # mutual upvote
    if instance.value == 1 and Vote.objects.filter(
        voter__id=instance.votee.id
    ).filter(votee__id=instance.voter.id).filter(value=1):
        # Register the match!
        Match.objects.create(
            matchingcat=instance.voter, matchedcat=instance.votee
        )
        Match.objects.create(
            matchingcat=instance.votee, matchedcat=instance.voter
        )
